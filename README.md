...
> [!IMPORTANT]
> beta.3の完了判定は、生成音声の聞き取り品質を受入条件に含めていなかったため撤回しました。beta.4では会話速度、ピッチ、促音、VOCALOID:AI Take、無音検出を実機で再検証しています。VOCALOID6と初音ミク V6は利用者自身が正規に所有・インストールする必要があります。プラグインはボイスバンク、立ち絵素材、認証情報を同梱しません。
...
- 日本語の漢字かな読み変換、モーラ分割、平坦ピッチと疑問末尾だけを上げる会話向け音程生成
- 促音の閉口区間化、VOCALOID:AI Take 1から10の選択、実機比較で選んだTake 10の既定値
...
4. 初期値の`Automatic`、テンポ`130`、テイク`10`で生成する。自動操作に失敗した場合だけ`Assisted`へ切り替える
...
機械的なビルド・自動操作・無音・口パク検査に加え、beta.4ではWhisper mediumによる聞き取り比較を記録しています。最終リリース判定には実際のリスニング受入を必要とします。詳細は[E2E検証証跡](docs/E2E_EVIDENCE.md)を参照してください。
